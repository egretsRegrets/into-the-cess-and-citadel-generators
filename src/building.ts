...
import { d6InteractionType } from "./adventure-site";
...
// saving this in case it comes in handy but the IC&C rooms aren't super useful
// gettting the rooms is made complicated by an attempt to limit repeat reooms, currently this is not done between floors
async function getRoomsFromRollResults(rollResults: number[]) {
...
function drawFloorPlan(
  ...
  const roomWithStairs: number | null = hasStairs
    ? getRandomInt(0, roomInteractions.length)
    : null;
...
export const genBuilding = async (
  ...
  const roomInteractionRollsFirstFloor = roll.d6(roomCountFirstFloor);
  const floorplan = getFloorPlan(roomCountFirstFloor);
  try {
    const roomInteractions: ASNodeInteraction[] =
      roomInteractionRollsFirstFloor.map((roll) => d6InteractionType(roll));

    let secondFloor = {};
    if (roomCountSecondFloor) {
      const roomInteractionRollsSecondFloor = roll.d6(roomCountSecondFloor);
      const secondFloorRoomInteractions = roomInteractionRollsSecondFloor.map(
        (roll) => d6InteractionType(roll),
      );
      ...
      secondFloor = {
        secondFloor: {
          rooms: secondFloorRoomInteractions,
...
${sedondFloorFloorplanDrawing}
    ...
    return {
      ...
      floorplan,
      floorplanDrawing: `
${drawFloorPlan(floorplan, roomInteractions, !!roomCountSecondFloor, 1)}
  `,
      ...secondFloor,
    };
  } catch (err) {
